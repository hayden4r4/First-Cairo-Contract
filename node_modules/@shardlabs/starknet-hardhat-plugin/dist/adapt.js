...
exports.adaptOutput = exports.adaptInput = void 0;
 ...
 */
function adaptInput(functionName, input, inputSpecs, abi) {
    const adapted = [];
    ...
    for (let i = 0; i < inputSpecs.length; ++i) {
        ...
        if (inputSpec.type === "felt") {
            ...
            else if (inputSpec.name.endsWith(constants_1.LEN_SUFFIX)) {
                const nextSpec = inputSpecs[i + 1];
                const arrayName = inputSpec.name.slice(0, -constants_1.LEN_SUFFIX.length);
                if (nextSpec && nextSpec.name === arrayName && nextSpec.type === "felt*" && arrayName in input) {
                    // will add array length in next iteration
                }
                else {
                    throw new plugins_1.HardhatPluginError(constants_1.PLUGIN_NAME, errorMsg);
                }
            }
            else {
                throw new plugins_1.HardhatPluginError(constants_1.PLUGIN_NAME, errorMsg);
            ...
            const lenName = `${inputSpec.name}${constants_1.LEN_SUFFIX}`;
...
function adaptComplexInput(input, inputSpec, abi, adaptedArray) {
    ...
    if (input === undefined || input === null) {
        throw new plugins_1.HardhatPluginError(constants_1.PLUGIN_NAME, `${inputSpec.name} is ${input}`);
    }
    if (type === "felt") {
        if (isNumeric(input)) {
    ...
    // otherwise a struct
    if (!(type in abi)) {
        throw new plugins_1.HardhatPluginError(constants_1.PLUGIN_NAME, `Type ${type} not present in ABI.`);
    }
    const generatedComplex = {};
 ...
 */
function adaptOutput(rawResult, outputSpecs, abi) {
    ...
    for (const outputSpec of outputSpecs) {
        ...
        if (outputSpec.type === "felt") {
            ...
            const lenName = `${outputSpec.name}${constants_1.LEN_SUFFIX}`;
            if (lastSpec.name !== lenName || lastSpec.type !== "felt") {
...
exports.adaptOutput = adaptOutput;
/**
